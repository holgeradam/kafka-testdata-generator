...
func (g *Generator) value(schema map[string]any, path string, depth int) (any, error) {
	schema = normalizeSchema(schema)

	if ref, ok := schema["$ref"].(string); ok {
		return g.refValue(ref, path, depth)
	}
	...
	if allOf, ok := schema["allOf"].([]any); ok {
		return g.mergeAllOf(allOf, path, depth)
	}
	...
	case "array":
		return g.array(schema, path, depth)
	case "string":
		return g.string(schema, leafName(path), path)
	case "integer":
...
// leafName returns the trailing field name of a JSON Path fragment, for use in
// field-name heuristics. "$.order.email" -> "email"; "$.items[0].sku" -> "sku";
// the bare root "$" yields "".
...
func (g *Generator) refValue(ref, path string, depth int) (any, error) {
	if g.resolveRef == nil {
	...
	return g.value(target, path, depth+1)
}
...
func (g *Generator) object(schema map[string]any, path string, depth int) (any, error) {
	...
	for _, name := range names {
		...
		if g.shouldInclude(name, required) {
			v, err := g.value(ps, path+"."+name, depth)
			if err != nil {
...
func (g *Generator) array(schema map[string]any, path string, depth int) (any, error) {
	...
	for i := 0; i < count; i++ {
		item, err := g.value(items, fmt.Sprintf("%s[%d]", path, i), depth)
		if err != nil {
			if errors.Is(err, errAbsent) {
				continue
			}
