...
Amended (2026-09-20, ADR-0009): the missing-Key skip is gone. The Key is no longer extracted from the Payload but generated from the key schema and planted at `-keyPath`, and a path that is not guaranteed in every record is refused at startup. A Key that cannot be produced is therefore universal, like a schema failure, and aborts the run; `Failed` again counts only sink send errors.
